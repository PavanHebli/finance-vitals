...
# ASCII-safe substitutions for core Helvetica font (Latin-1 only)
_REPLACEMENTS = [
    ("–", "-"),    # en dash
    ("—", "-"),    # em dash
    ("≥", ">="),   # >=
...
def generate_pdf(
    ...
    from modules.education import get_education
    ...
    W = pdf.w - 40  # usable width (170 mm on A4)
    ...
    # ── Overall score ─────────────────────────────────────────────────────────────
    # Extra top padding so the large glyph ascender clears the header text
    label = mirror.get("label", "")
    lr, lg, lb = _LABEL_COLORS.get(label, (100, 100, 100))

    pdf.set_font("Helvetica", "B", 52)
    pdf.set_text_color(30, 30, 30)
    # Cell height 28 comfortably holds a 52pt glyph (~18 mm) with room above/below
    pdf.cell(W, 28, str(overall_score), align="C")
    pdf.ln(28)  # advance past the cell explicitly so next element doesn't overlap
    ...
    total_expenses = sum(
        state.get(f"expenses_{k}", 0.0)
        for k in ["rent", "groceries", "transport", "subscriptions", "dining", "shopping", "other"]
    )
    ...
    numbers = [
        ...
        ("Monthly Expenses", f"${total_expenses:,.0f}"),
        ("Savings",          f"${savings:,.0f}"),
        ("Total Debt",       f"${debt_total:,.0f}"),
    ]
    ...
    pdf.multi_cell(W, 4, "Ratios are calculated using take-home (after-tax) income -- stricter than lender benchmarks, which use gross income.")

    # ── Page 2: Narrative + Why this matters ──────────────────────────────────────
    pdf.add_page()
    ...
    pdf.multi_cell(
        W, 4,
        "Generated by myfinancevitals.app  |  For informational purposes only - not financial advice. "
